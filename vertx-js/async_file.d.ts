/// <reference path="./globals.d.ts" />/// <reference path="./buffer" />/// <reference path="./write_stream" />/// <reference path="./read_stream" />declare module "vertx-js/async_file" {  export = AsyncFile;}
...
declare interface AsyncFile  extends      ReadStream  ,     WriteStream  {            
...
/**
   ...
   */
  writeQueueFull(): boolean;              handler(handler: (e: Buffer) => void): AsyncFile;              pause(): AsyncFile;              resume(): AsyncFile;              endHandler(endHandler: (e: void) => void): AsyncFile;              write(data: Buffer): AsyncFile;              setWriteQueueMaxSize(maxSize: number): AsyncFile;              drainHandler(handler: (e: void) => void): AsyncFile;              exceptionHandler(handler: (e: Throwable) => void): AsyncFile;              
 ...
 * the data will be read at a time from the file system.
   */
  setReadBufferSize(readBufferSize: number): AsyncFile;    }                                        declare var AsyncFile: {}